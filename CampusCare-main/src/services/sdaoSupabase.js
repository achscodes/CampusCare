...
export async function loadSdaoFromSupabase(supabase) {
  const [bRes, aRes, cRes, dRes, rRes, discRes, hsoRes] = await Promise.all([
    ...
    supabase.from("health_referrals").select("*").eq("receiving_office", "development").order("referral_date", { ascending: false }),
  ]);

  const err = bRes.error || aRes.error || cRes.error || dRes.error || rRes.error || discRes.error || hsoRes.error || null;
  if (err) {
    return {
      ok: false,
      error: err,
      ...
      healthReferralsIncoming: [],
    };
  }

  // Map health referrals to match expected structure
  const hsoReferralsMapped = (hsoRes.data || []).map((r) => ({
    id: r.id,
    ...
    reason: r.reason || "",
    observations: r.health_observations || "",
    recommendedAction: r.recommended_action || "",
    date: r.referral_date ? formatShortDate(new Date(`${r.referral_date}T12:00:00`)) : "—",
    dateSort: r.referral_date || "",
  ...
  return {
    ...
    healthReferralsIncoming: hsoReferralsMapped,
  };
}
