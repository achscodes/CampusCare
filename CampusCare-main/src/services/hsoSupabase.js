...
export async function loadHsoFromSupabase(supabase) {
  const [cRes, rRes, aRes, refRes, dRes, discRes, sdaoRes] = await Promise.all([
    ...
    supabase.from("sdao_referrals").select("*").eq("receiving_office", "health").order("created_at", { ascending: false }),
  ]);

  const err =
    cRes.error || rRes.error || aRes.error || refRes.error || dRes.error || discRes.error || sdaoRes.error || null;
  if (err) {
    return {
      ok: false,
      error: err,
  ...
  // Map SDAO referrals to match HSO referral structure (like rowToReferral output)
  const sdaoReferralsMapped = (sdaoRes.data || []).map((r) => ({
    referralId: String(r.id ?? ""),
    studentName: String(r.student_name ?? ""),
    studentId: String(r.student_id ?? ""),
    referralType: "SDAO Referral",
    reason: String(r.reason ?? ""),
    status: String(r.status ?? "sent"),
    date: r.created_at ? formatShortDate(`${r.created_at.split("T")[0]}T12:00:00`) : "—",
    evidence: Array.isArray(r.attachments) ? r.attachments : [],
    referringOffice: String(r.referring_office ?? "development").toLowerCase(),
    targetOffice: "health",
    program: r.program || "",
    email: r.email || "",
    phone: r.phone || "",
  ...
  return {
    ...
    sdaoReferralsIncoming: sdaoReferralsMapped,
  };
}
