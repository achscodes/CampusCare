...
export function formatAuthError(error) {
  ...
  if (/redirect.*url|redirect_to|invalid.*redirect/i.test(msg)) {
    return (
      "Password reset could not start because the return URL is not allowed. " +
      "Add this app’s /forgot-password URL under Supabase → Authentication → URL Configuration → Redirect URLs."
    );
  }

  if (/smtp|mail.*server|email.*provider|custom.*smtp/i.test(msg)) {
    return (
      "Password reset email is not configured on the server. " +
      "In Supabase → Authentication → SMTP Settings, enable custom SMTP (e.g. Resend: smtp.resend.com, user resend, password = your re_ API key)."
    );
  }

  if (/confirmation email|error sending.*email|send.*confirm|email.*could not be sent|error sending recovery|recovery email/i.test(msg)) {
    return (
      "We couldn’t send the password reset email. " +
      "Check Supabase → Authentication → Email (SMTP + Reset Password template), or try again in a few minutes."
    );
  }
