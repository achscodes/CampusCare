...
function SDAO({ embedDashboardOnly = false } = {}) {
  ...
  const [healthReferralsIncoming, setHealthReferralsIncoming] = useState([]);
  ...
  const refreshSdao = useMemo(() => {
    return async () => {
      ...
      setHealthReferralsIncoming(res.healthReferralsIncoming || []);
    };
  }, []);
  ...
  const renderReferrals = () => {
    ...
    return (
      ...
      <div className="cases-panel" style={{ marginTop: 24 }}>
        <div className="cases-panel-header">
          <div className="cases-panel-title">Incoming from Health Services ({healthReferralsIncoming.length})</div>
          <p className="hs-list-sub" style={{ margin: "4px 0 0" }}>
            Approve or decline referrals from Health Services Office sent to SDAO.
          </p>
        </div>
        <div className="cases-table-wrapper">
          <table className="cases-table">
                ...
                <tr key={r.id}>
                  ...
                  <td>
                    <button
                      ...
                      className="hs-link-action sdao-link-referral"
                      onClick={() => {
                        setSelectedReferral({ ...r, healthIncoming: true });
                        setActiveModal("referralDetails");
                      }}
                    >
                      <Eye size={14} strokeWidth={1.5} aria-hidden />
          ...
          {healthReferralsIncoming.length === 0 ? (
            <p className="hs-list-sub" style={{ padding: "16px 12px", margin: 0 }}>
              No incoming referrals from Health Services.
            </p>
          ) : null}
        </div>
      </div>
