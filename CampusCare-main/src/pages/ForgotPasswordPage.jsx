...
import { requestPasswordRecoveryOtp } from "../services/passwordRecoveryOtp";

/** Default when using Supabase Auth SMTP; Edge Function may return a different length. */
const DEFAULT_RECOVERY_OTP_LENGTH = 8;

/**
 * Password reset: Edge Function `password-recovery-otp` emails the code (Resend);
 * then verifyOtp({ email, token, type: 'recovery' }) and updateUser({ password }).
 */
const ForgotPasswordPage = () => {
  ...
  const [otpLength, setOtpLength] = useState(DEFAULT_RECOVERY_OTP_LENGTH);
  const [codeDigits, setCodeDigits] = useState(
    Array.from({ length: DEFAULT_RECOVERY_OTP_LENGTH }, () => ""),
  );
  ...
  const handleSendCode = async (e) => {
    ...
    if (isSupabaseConfigured() && supabase) {
      ...
      devLog("[AUTH] Sending password recovery OTP via Edge Function:", trimmed);
      const sendResult = await requestPasswordRecoveryOtp(trimmed);
      setSendingCode(false);

      if (!sendResult.ok) {
        devWarn("[AUTH] Password recovery email failed:", sendResult.error);
        setFieldErrors({ email: sendResult.error });
        return;
      }

      const len = sendResult.otpLength || DEFAULT_RECOVERY_OTP_LENGTH;
      setOtpLength(len);
      devLog("[AUTH] Password recovery email sent, otpLength:", len);
      setCodeDigits(Array.from({ length: len }, () => ""));
      setStep("code");
    ...
    setOtpLength(DEFAULT_RECOVERY_OTP_LENGTH);
    setCodeDigits(Array.from({ length: DEFAULT_RECOVERY_OTP_LENGTH }, () => ""));
    setStep("code");
  ...
  const handleVerifyCode = async (e) => {
    ...
    if (
      ...
      !new RegExp(`^\\d{${otpLength}}$`).test(code)
    ) {
      nextErrors.code = `Enter the ${otpLength}-digit verification code.`;
    }
