...
import LoginOtpPendingGuard from "./components/auth/LoginOtpPendingGuard";
...
function mustChangePasswordAllowedPath(pathname) {
  const p = pathname || "";
  if (["/signin", "/forgot-password", "/terms", "/privacy", "/signup"].includes(p)) return true;
  if (
    p === "/do/profile-settings" ||
    p === "/sdao/profile-settings" ||
    p === "/health-services/profile-settings"
  ) {
    return true;
  }
  return false;
...
function MustChangePasswordGate({ children }) {
  ...
  if (
    ...
    !mustChangePasswordAllowedPath(location.pathname)
  ) {
    const ps = profileSettingsPathForSessionOffice(session.office);
    if (typeof ps !== "string" || !ps) {
      return children;
    }
    return <Navigate to={`${ps}?password=required`} replace />;
  }
